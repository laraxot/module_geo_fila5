 ...
 * - La colonna valutatore_id viene aggiunta nella sezione di update SOLO SE NON ESISTE.
        ...
        $this->tableUpdate(
            function (Blueprint $table): void {
                ...
                if (! $this->hasColumn('importo_totale_valutatore_id')) {
                    $table->decimal('importo_totale_valutatore_id', 15, 2)->nullable()->comment('Importo finale per valutatore_id');
                }
                // Aggiunta colonna resti_pond_valutatore_id se non esiste
                if (! $this->hasColumn('resti_pond_valutatore_id')) {
                    $table->decimal('resti_pond_valutatore_id', 15, 2)->nullable()->comment('Resti ponderati calcolati per valutatore_id');
                }
            });
    }
};
