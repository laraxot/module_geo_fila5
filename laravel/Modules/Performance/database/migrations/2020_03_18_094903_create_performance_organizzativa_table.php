 ...
 * Le colonne valutatore_id, importo_totale_valutatore_id e resti_pond_valutatore_id vengono gestite qui secondo le regole WindSurf/Laraxot:
 *  - Non si crea una nuova migrazione di update, ma si aggiorna la migrazione principale con timestamp aggiornato.
 *  - Ogni modifica strutturale va documentata in docs/database_migrations.md e collegata alla root docs.
 *  - Motivazione: evitare errori SQL, duplicazioni e garantire compliance PHPStan livello 10.
    ...
    public function up(): void
    {
        ...
        $this->tableUpdate(
            function (Blueprint $table): void {
                ...
                // Aggiunta colonna importo_totale_valutatore_id se non esiste
                if (! $this->hasColumn('importo_totale_valutatore_id')) {
                    $table->decimal('importo_totale_valutatore_id', 15, 5)->nullable()->after('importo_totale');
                }
                // Aggiunta colonna resti_pond_valutatore_id se non esiste
                if (! $this->hasColumn('resti_pond_valutatore_id')) {
                    $table->decimal('resti_pond_valutatore_id', 15, 5)->nullable()->after('resti_pond');
                }
            });
    }
};
