...
# Gdpr Module — PHPStan Type Compliance
...
```bash
vendor/bin/phpstan analyse laravel/Modules/Gdpr \
  --level=max \
  --no-progress
```
...
```bash
vendor/bin/phpstan analyse laravel/Modules/Gdpr --level=max
```
...
# Verbose mode
vendor/bin/phpstan analyse laravel/Modules/Gdpr --level=max -v
```
...
```bash
vendor/bin/pest laravel/Modules/Gdpr/tests --parallel
```
