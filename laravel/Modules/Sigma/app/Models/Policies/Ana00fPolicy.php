...
use Modules\Sigma\Models\Ana00f as Model;
use Modules\Xot\Actions\Cast\SafeEloquentCastAction;
use Modules\Xot\Contracts\UserContract;
use Modules\Xot\Models\Policies\XotBasePolicy;

class Ana00fPolicy extends XotBasePolicy
{
    /**
    ...
    {
        if ($user->hasRole(['super-admin', 'admin', 'hr-manager', 'evaluator'])) {
            return true;
        }

        $profile = $user->profile()->first();
        if (! ($profile instanceof EloquentModel)) {
            return false;
        }

        $userMatr = app(SafeEloquentCastAction::class)->getStringAttribute($profile, 'matr', '');
        $modelMatr = (string) ($model->matr ?? '');

        return $userMatr !== '' && $userMatr === $modelMatr;
    }
