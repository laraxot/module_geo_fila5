...
# Linee Guida AI PTVX - Refactor DRY + KISS

> **PRINCIPIO FONDAMENTALE**: Tutto il codice PTVX deve estendere le classi base Xot, mai direttamente le classi Laravel/Filament.

## 🎯 **STRUTTURA DRY + KISS**

Questo refactor elimina completamente le duplicazioni e centralizza tutte le informazioni in file dedicati:

### 📚 **File Centralizzati**

1. **[CORE.md](CORE.md)** - Regole critiche e fondamentali
2. **[PATTERNS.md](PATTERNS.md)** - Pattern architetturali e best practices
3. **[DEVELOPMENT.md](DEVELOPMENT.md)** - Sviluppo, testing e qualità
4. **[FILAMENT.md](FILAMENT.md)** - Filament specifico
5. **[ARCHITECTURE.md](ARCHITECTURE.md)** - Architettura e struttura

## 🚨 **REGOLE CRITICHE IMMEDIATE**
...
### 3. **VIETATO USO DI ->label(), ->placeholder(), ->helperText()**
...
TextInput::make('name')->label('Nome');  // VIETATO
```
...
1. Leggi **[CORE.md](CORE.md)** per le regole fondamentali
2. Consulta **[ARCHITECTURE.md](ARCHITECTURE.md)** per la struttura
3. Usa **[PATTERNS.md](PATTERNS.md)** per i pattern comuni

### Per Filament

1. Leggi **[FILAMENT.md](FILAMENT.md)** per tutte le regole Filament
2. Segui le best practices per Resources, Actions e Widget

### Per Testing e Qualità

1. Consulta **[DEVELOPMENT.md](DEVELOPMENT.md)** per testing e PHPStan
2. Segui le checklist per ogni tipo di sviluppo

## 📋 **CHECKLIST RAPIDA**

### Prima di ogni Commit

- [ ] Codice estende classi Xot base
...
- Zero duplicazioni tra file
- Regole centralizzate e aggiornate
- Esempi coerenti e riutilizzabili

### ✅ **KISS (Keep It Simple, Stupid)**

- Struttura lineare e intuitiva
- File dedicati per ogni ambito
...
Inizia sempre da **[CORE.md](CORE.md)** per capire le regole fondamentali.

### 2. **Consultazione Specifica**

Usa i file dedicati per l'ambito specifico:
...
Consulta **[PATTERNS.md](PATTERNS.md)** per esempi riutilizzabili.

### 4. **Aggiornamenti**

Ogni aggiornamento viene fatto nel file appropriato, mantenendo la coerenza.

## 🔄 **VERSIONING E AGGIORNAMENTI**

- **Versione**: 2.0 (Refactor DRY + KISS)
- **Data**: Giugno 2025
- **Stato**: Completamente centralizzato
- **Duplicazioni**: Eliminate al 100%

## 📞 **SUPPORTO E FEEDBACK**

Per domande o suggerimenti:
1. Consulta prima i file appropriati
2. Verifica che non sia già documentato
3. Proponi miglioramenti per i file specifici

---

**Ricorda**: Queste linee guida sono la fonte unica di verità per PTVX. Seguile sempre per garantire coerenza e qualità del codice.
...
## ⚠️ **REGOLA CRITICA: Gestione Proprietà Modelli Eloquent**
...
=== .ai/eloquent_models_property_verification rules ===
...
// ❌ VIETATO - getTableColumns() va solo nelle pagine List, MAI nelle Resource
public function getTableColumns(): array { /* VIETATO */ }

// ❌ VIETATO - Non sovrascrivere questi metodi
public function form(\Filament\Schemas\Schema $form): \Filament\Schemas\Schema { /* VIETATO */ }
public function table(Table $table): Table { /* VIETATO */ }
```

### 4. **COMPONENTI CUSTOM SELECT - REGOLE FONDAMENTALI**

```php
// ✅ CORRETTO - Componente select custom PTVX
class SelectValutatore extends Select
{
    ...
    protected function setUp(): void
    {
        ...
        $this->options(function () {
            // Determina dinamicamente il modello in base al modulo chiamante
            $module_name = $this->getCallingModuleName();
            $model = 'Modules\\' . $module_name . '\\Models\\StabiDirigente';
            
            $data = [];
            $rows = $model::where($this->getWhere())->get();
            foreach ($rows as $row) {
...
// ❌ VIETATO - Errori comuni da evitare
// 1. Non usare Auth::user()->teams() in componenti custom
// 2. Non passare parametri alla closure options()
// 3. Non usare query complesse invece di Model::where()
```
