---
title: "Geo Module - PHPStan Type Compliance"
type: concept
tags: [geo, phpstan, types, compliance, quality, static-analysis]
created: 2026-06-10
updated: 2026-06-10
related:
...
# Geo Module — PHPStan Type Compliance
...
```bash
vendor/bin/phpstan analyse laravel/Modules/Geo \
  --level=max \
  --no-progress
```
...
```bash
vendor/bin/phpstan analyse laravel/Modules/Geo --level=max
```
...
# Verbose mode
vendor/bin/phpstan analyse laravel/Modules/Geo --level=max -v
```
...
```bash
vendor/bin/pest laravel/Modules/Geo/tests --parallel
```
