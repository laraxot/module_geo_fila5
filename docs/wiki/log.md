...
[2026-06-15] [FIX] Sigma — `Qua00f` e `Rep00f` rimossa ridichiarazione `implements Contracts\SigmaDateRangeFields` (ereditata da `BaseDateRangeModel`). Nuova regola locale `contract-inheritance-no-redeclare.md`. Allineato a `class-inheritance-principles.md` + `inheritance-composition-no-duplication.md`. Metadati+indici aggiornati. llm-wiki ingest.
