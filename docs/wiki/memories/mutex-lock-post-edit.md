...
Per edited path: companion `file.ext.lock` only if absent; remove after edit. PHP in `laravel/`: PHPStan L10 + `laravel/tools/phpmd.sh` + `laravel/tools/phpinsights.sh`.
