...
## Implementazione
La funzione supporta l'esecuzione in ambiente CI (es. GitHub Actions) saltando il prompt interattivo se la variabile d'ambiente `CI` è impostata.

```bash
if [ -z "$CI" ]; then
    backup_disk
else
    echo "🔄 CI environment detected, skipping backup..."
fi
```
...
- [GitHub Actions](../git/github-actions.md) - Automazione sync remoto
