...
```text
# SISTEMA DI DOCUMENTAZIONE LARAXOT – GUIDA COMPLETA

## 📋 INDICE GENERALE

### 1. ARCHITETTURA E STACK TECNOLOGICO
### 2. STRUTTURA GERARCHICA E MODULARITÀ
...
- **Geo**: Dati geografici e località (opzionale)
...
- **Rule files**: `.windsurf/rules/`, `.cursor/rules/` – regole IDE e policy.
...
- **ERRORE COMUNE**: `Modules\{ModuleName}\Filament\` è SBAGLIATO
...
- **Motivazione e "perché"**: ogni regola deve spiegare la motivazione.
...
- **Entity Management**: Gestione entità con enum states
- **Media Library**: Spatie Media Library per file uploads (collection pattern)
- **Geographic Data**: Integrazione modulo Geo per località (se presente)
- **Multi-tenant**: Filament tenancy per isolamento dati
- **Single Table Inheritance**: Tighten Parental per tipi entità
...
- [ ] Assicura compatibilità HTML2PDF dove necessario
