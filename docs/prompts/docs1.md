...
# Regole Operative Complete per Laraxot
# Documentazione Aggiornata - Analisi Approfondita del Codice
...
### 7. DOCUMENTAZIONE
### 8. WORKFLOW E TOOLS
### 9. FRONTEND E COMPONENTI
### 10. ENUM E CASTING
### 11. HTML2PDF E GENERAZIONE DOCUMENTI
### 12. REGOLE CRITICHE - RIEPILOGO
### 13. RIFERIMENTI E COLLEGAMENTI

---

## 1. ARCHITETTURA E STRUTTURA

### 1.1 Stack Tecnologico Laraxot
...
```
/var/www/html/_bases/base_{project}/
...
- **Enum**: PascalCase con suffisso Enum (es. `{ModelName}StatusEnum`)
...
### 3.4 Array Associativi OBBLIGATORI
Tutti i metodi che restituiscono array devono usare chiavi stringa:
...
### 3.5 Rimozione Override Inutili
**REGOLA CRITICA**: Rimuovere override che restituiscono solo valori standard

```php
// ❌ ERRATO - Rimuovere se restituisce solo azioni standard
public static function getTableActions(): array
{
    return [
...
// ✅ CORRETTO - Rimuovere completamente il metodo
// Le azioni standard vengono ereditate automaticamente
```
...
**OBBLIGATORIO**: Tutti gli URL devono includere il prefisso lingua
...
```php
use Modules\{ModuleName}\Enums\{ModelName}StatusEnum;

public function test_{modelName}_status_enum()
{
    $status = {ModelName}StatusEnum::CONFIRMED;
...
### 6.5 Foreign Keys
Utilizzare sempre `foreignIdFor()`:
...
### 7.3 Collegamenti Bidirezionali
...
### 7.4 Aggiornamento Documentazione
...
### 10.1 Enum con Select Components
**OBBLIGATORIO**: Utilizzare sempre classi Enum con Select:

```php
// ✅ CORRETTO
Components\Select::make('status')
    ->options({ModelName}StatusEnum::class)
    ->default({ModelName}StatusEnum::SCHEDULED)
    ->required();
...
### 10.2 Casting di Modelli
**OBBLIGATORIO**: Utilizzare sempre metodo `casts()` invece della proprietà `$casts`:
...
protected function casts(): array
{
    return [
        'created_at' => 'datetime',
        'is_active' => 'boolean',
        'status' => {ModelName}StatusEnum::class, // enum
        'options' => 'array',
    ];
}
```
...
### 10.3 Campi Booleani
Tutti i campi booleani devono essere esplicitamente castati:
...
- ❌ Percorsi assoluti nei link documentazione
- ❌ Componenti UI in `resources/views/components/`
- ❌ Layout inesistenti come `<x-filament::layouts.app>`
...
- ✅ Enum per stati e valori predefiniti
- ✅ Percorsi relativi nella documentazione
- ✅ Componenti UI in `Modules/UI/resources/views/components/ui/`

### 12.3 Checklist Operativa
- [ ] Namespace senza 'App'
- [ ] Estensione BaseModel del modulo
- [ ] Strict types in tutti i file
- [ ] getFormSchema() implementato
- [ ] NO ->label() nei componenti
- [ ] Struttura espansa traduzioni
- [ ] PHPStan livello 9+ passato
- [ ] Test senza RefreshDatabase
- [ ] Documentazione aggiornata
- [ ] File docs in minuscolo
- [ ] Array associativi con chiavi stringa
- [ ] Enum utilizzati correttamente
- [ ] Percorsi relativi nella documentazione
- [ ] Componenti UI nel posto giusto
