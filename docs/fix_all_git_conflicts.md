...
### 1. Script Principale (V5.0) - CONSIGLIATO
**Posizione**: `bashscripts/git/conflict_resolution/resolve_conflicts_current_change.sh`

Il nuovo script ottimizzato con le seguenti caratteristiche:
- ✅ Risoluzione automatica scegliendo sempre la "current change"
- ✅ Modalità dry-run per testare prima di applicare
- ✅ Gestione avanzata dei file binari
...
**Utilizzo:**
```bash
# Test dry-run (consigliato)
./bashscripts/git/conflict_resolution/resolve_conflicts_current_change.sh --dry-run --verbose

# Esecuzione reale
./bashscripts/git/conflict_resolution/resolve_conflicts_current_change.sh

# Con output verbose
./bashscripts/git/conflict_resolution/resolve_conflicts_current_change.sh --verbose
```

### 2. Script Legacy
- `fix_git_conflicts_current_change.sh` (V4.0)
- `fix_git_conflicts_current_change_v2.sh` (V3.1)

## Revisione Manuale
File aggiornato per includere il nuovo script V5.0 ottimizzato posizionato correttamente nella sottocartella git/conflict_resolution/. Vedi anche [README globale](/docs/README.md) e [scripts_conflict_resolution.md](scripts_conflict_resolution.md).
...
- [Gestione Conflitti Git](/var/www/html/_bases/base_ptvx_fila5_mono/bashscripts/docs/git_conflicts_resolution.md)
